...
stack_snapshot(
    ...
    extra_deps = {
        # XXX: Using zlib.dev causes linker errors when building hie-bios. The
        # zlib shared object uses the execroot for an RPATH. Fix that.
        "zlib": ["@zlib.win//:zlib"],
    },
    haddock = False,
...
cc_library(name = "z", srcs = glob(["*.c"]), hdrs = glob(["*.h"]), linkstatic = True)
""",
